...
public class BusService {
    ...
    public Bus obtenerBusPorId(UUID id) {
        return busRepository.findById(id).orElseThrow(()-> new RuntimeException("Bus no encontrado"));
    }
    ...
    public void actualizarBus(UUID id, Bus bus) {
        Bus busActual = busRepository.findById(id).orElseThrow(()-> new RuntimeException("Bus no encontrado"));

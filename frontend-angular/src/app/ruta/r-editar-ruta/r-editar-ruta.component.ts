...
export class REditarRutaComponent {
  ...
  deleteRoute(event: Event) {
    if (this.ruta && this.ruta.id) {
      // Verificar si la ruta tiene estaciones asignadas
      if (this.estacionesAsignadas.length > 0) {
        this.messageService.add({
          severity: 'error',
          summary: 'No se puede eliminar',
          detail: 'No se puede eliminar la ruta porque aún tiene estaciones asignadas.',
          life: 3000
        });
        return;
      }
